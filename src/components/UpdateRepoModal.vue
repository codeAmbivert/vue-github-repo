    ...
    <div class="p-3 max-w-3xl w-full bg-white rounded-lg m-5">
      ...
      <div class="flex flex-col justify-center items-center gap-3 mt-5">
        <div
          ...
          <input type="text" v-model="formData.name" placeholder="New repository name"
...
export default {
  ...
  setup(props) {
    ...
    const disable = computed(() => !formData.value.name);
    ...
    const handleCreateRepo = async () => {
      loading.value = true;
      try {
        await axios.post(`https://api.github.com/repos/codeAmbivert/${props.repo}`, formData.value, {
          headers: {
